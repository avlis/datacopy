...
- CONNECTIONS_FILE: (default connections.csv) (can also be passed on the command line with -c or --connections-file)
- JOB_FILE: the csv file (default jobs.csv) (can also be passed with -j or --job-file )
- LOG_NAME: the output files name prefix. (defaults to timestamp) (can also be passed as -j or --log-file)
- TEST_QUERIES: set to 'yes' to only execute the select, and does not delete/write on destinations. same as testQueries command.
...
- DEBUG_MODULES: to specify the module you want more detail on: jobs, jobmanager, datahandlers, connections
...
## Commands
The first parameter passed to the script (or container) is considered the command.
The default command, if ommited, is "copy".
...
- testQueries: This will test the queries in the configuration files only, will not copy any data.
- createTables: calls an LLM to generate the create statement for target databases based on the output of the queries. activates testQueries mode.
...
## Experimental: AI assist for destination table creation: createTables command.

A new command was added, to facilitate the creation of destination tables.
If the command createTables is used, the testQueries mode is also activated, and the metadata of the resulting columns is sent to an LLM, to generate a CREATE TABLE command compatible with the destination database.
This feature will talk with a openAI compatible chat endpoint.

for this to work, two parameters need to be supplied by environment variables or command line:
- AI_API_ENDPOINT or --ai-api-endpoint: something like http://my-lmstudio-server:1234:/v1/chat/completions
- AI_API_MODEL or --ai-api-model: something like mlx-community/Qwen3-Coder-30B-A3B-Instruct-8bit
- AI_API_KEY or --ai-api-key: optional, just in case you are using a clould based solution that requires it.

the generated commands WILL NOT be executed on the target database, will only be written to the log:
```
(...)
2026-02-14 14:48:24.082156:INFO:[modules.connections.initConnections(474)]: (SEP): connected
2026-02-14 14:48:24.137163:INFO:[modules.jobmanager.jobManager(353)][2-SEP-PG-GRUPO_EFR_loading]: reading data from [SEP] with query:
***
SELECT * FROM GRUPO_EFR
***
2026-02-14 14:48:24.195490:INFO:[modules.ai.generate_create_table(33)]: called, create statement for [postgres] target, from [mysql] source
2026-02-14 14:48:31.022602:INFO:[modules.jobmanager.jobManager(492)][2-SEP-PG-GRUPO_EFR_loading]: create statement generated by LLM:[
DROP TABLE IF EXISTS GRUPO_EFR_loading;
CREATE TABLE GRUPO_EFR_loading (
    COD_GRUPO_IG VARCHAR(10),
    DESCR_GRUPO_IG VARCHAR(50),
    DATA_SOURCE VARCHAR(20) NOT NULL,
    DT_CRI_SO DATE,
    DT_CRI_DW DATE
);
]
2026-02-14 14:48:35.062119:INFO:[modules.jobmanager.jobManager(689)]: statement(s) executed in 11.30 seconds
```

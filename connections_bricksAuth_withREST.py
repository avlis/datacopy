    ...
    match c['driver']:
        case 'pyodbc':
            try:
                ...
                for x in range(p_qtd):
                    ...
                    nc[x]=pyodbc.connect(f"DRIVER={{ODBC Driver 18 for SQL Server}};SERVER={c['server']};DATABASE={{{c['database']}}};UID={{{c['user']}}};PWD={{{c['password']}}};ENCODING=UTF-8;TRUSTSERVERCERTIFICATE={c['trustservercertificate']};APP=datacopy")
        ...
        case 'cx_Oracle':
            try:
                ...
                for x in range(p_qtd):
                    ...
                    try:
                        nc[x].client_identifier='datacopy'
                    except Exception as e:
                        logging.logPrint(f'initConnections({p_name}): could not set client_identifier on connection: [{e}]', shared.L_DEBUG)
        ...
        case 'databricks':
            try:
                ...
                logging.logPrint(f'databricks auth: trying to get auth token with client_id=[{client_id}], client_secret [{client_secret}], server_name=[{server_name}]', shared.L_DEBUG)
                try:
                    response = requests.post(
                        url=f'https://{server_name}/oidc/v1/token',
                        auth=(client_id, client_secret),
                        data={'grant_type':'client_credentials','scope':'sql'},
                        verify=shared.AllowInsecureWebRequests
                    )
                except Exception as e:
                    logging.logPrint(f'databricks auth get_token: exception [{e}]')
                
                logging.logPrint(f'databricks auth: got response [{response.status_code}]: [{response.reason}]', shared.L_DEBUG)
                response.raise_for_status()  # Raise an exception for bad status codes
                access_token = response.json()["access_token"]
                logging.logPrint(f'databricks auth: got acces_token [{access_token}]', shared.L_DEBUG)

                for x in range(p_qtd):
                    logging.logPrint(f'databricks[{x}]: establishing connection...', shared.L_DEBUG)
                    ...
                    logging.logPrint(f'databricks[{x}]: connected.', shared.L_DEBUG)
            except Exception as error:
                logging.logPrint(f'initConnections({p_name}): DB error [{error}]')
                shared.ErrorOccurred.value=True
                logging.closeLog(2)
